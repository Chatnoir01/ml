...
class ManagedKeyService:
    ...
    def rotate(self, handle: str) -> ManagedKey:
        """Create replacement first, then retire old key under one service lock."""
        with self._lock:
            current = self._registry.get(handle)
            if current is None or current.state is not StoredKeyState.ACTIVE:
                raise LifecycleError("only active managed keys may rotate")
            if not self._provider.has_key(handle):
                raise LifecycleError("provider/registry divergence detected")
            txid = self._journal.begin(operation="rotate", handle=handle)
            replacement = self._provider.create_key()
            try:
                new_record = self._registry.register(
                    handle=replacement.handle, provider_id=self._provider.provider_id
                )
                self._registry.revoke(handle)
            except Exception:
                self._provider.destroy(replacement.handle)
                raise
            self._journal.commit(txid, operation="rotate", handle=handle)
            return ManagedKey(new_record, replacement.public_key_raw)
