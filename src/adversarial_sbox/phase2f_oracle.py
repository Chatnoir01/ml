"""Phase 2F fitness-Block-G scorer only.

Held-out Block X is deliberately absent. The evolutionary runner may import this
module, but it must never import the separate Phase-2F validation module.
"""
...
def _score_candidate(
    ...
    *,
    ...
    if len(dataset_seeds) != 8 or len(model_seeds) != 8:
        raise ValueError("Phase 2F neural scorer requires exactly eight paired seeds")
    if len(set(int(v) for v in dataset_seeds)) != 8 or len(set(int(v) for v in model_seeds)) != 8:
        raise ValueError("Phase 2F neural seed block contains duplicates")
    if purpose not in {"fitness", "validation"}:
        raise ValueError(f"unsupported Phase 2F neural purpose {purpose!r}")

    cipher = ToySPN(frozen, ROUND_KEYS[: DEPTH + 1])
    if cipher.rounds != DEPTH:
        raise RuntimeError("Phase 2F ToySPN depth drift")
    ...
    for difference in DIFFERENCES:
        for replicate, (dataset_seed, model_seed) in enumerate(zip(dataset_seeds, model_seeds)):
            ...
            if sizes != SPLIT_SIZES:
                raise RuntimeError(f"Phase 2F split-size drift: {sizes} != {SPLIT_SIZES}")
    ...
    if len(runs) != ORACLE_TRAININGS_PER_SCORE:
        raise RuntimeError("Phase 2F neural training-count drift")
    ...
    payload["scientific_payload_sha256"] = hashlib.sha256(_canonical_without_receipt(payload)).hexdigest()
    return payload


def score_fitness_candidate(sbox: Sequence[int]) -> dict[str, Any]:
    """Score one candidate using only fresh preregistered Phase-2F Block G."""
