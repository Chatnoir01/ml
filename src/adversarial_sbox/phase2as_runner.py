"""Scientific runner for the frozen Phase 2A-S depth-attenuation diagnostic.
...
def load_frozen_panel() -> tuple[dict[str, Any], ...]:
    ...
    if COMMITTED_DIGEST != PANEL_DIGEST_SHA256:
        raise RuntimeError("Phase 2A-S panel digest drift")
    if len(CANDIDATES) != CANDIDATE_COUNT:
        raise RuntimeError("Phase 2A-S panel size drift")
...
def _verify_candidate(candidate_index: int) -> tuple[tuple[int, ...], dict[str, Any]]:
    ...
    if not 0 <= int(candidate_index) < len(panel):
        raise ValueError("Phase 2A-S candidate index out of range")
    ...
    if not all(checks.values()):
        raise RuntimeError(
            f"Phase 2A-S candidate {candidate_index} failed classical revalidation: {checks}"
        )
    return sbox, {
...
def _cell_spec(depth: int, input_difference: int) -> tuple[int, int]:
    ...
    if depth not in DEPTHS:
        raise ValueError(f"undeclared Phase 2A-S depth {depth}")
    if difference not in DIFFERENCES:
        raise ValueError(f"undeclared Phase 2A-S difference {difference:#x}")
...
def run_cell(depth: int, input_difference: int) -> dict[str, Any]:
    """Run one frozen 48-training depth/difference cell."""
    ...
    if len(runs) != TRAININGS_PER_CELL:
        raise RuntimeError("Phase 2A-S cell training count drift")
...
def _candidate_means(blocks: Sequence[Sequence[float]]) -> list[float]:
    if not blocks or any(len(block) != CANDIDATE_COUNT for block in blocks):
        raise ValueError("Phase 2A-S blocks must be non-empty and panel-width")
    return [
...
def aggregate_cells(results: Sequence[dict[str, Any]]) -> dict[str, Any]:
    """Aggregate all six cells and apply the frozen attenuation classification."""

    expected_keys = expected_cell_keys()
    if len(results) != len(expected_keys):
        raise ValueError("Phase 2A-S aggregation requires exactly six cell results")
    ...
    for item in results:
        ...
        if key in cells:
            raise ValueError(f"duplicate Phase 2A-S cell {key!r}")
        if key not in expected_keys:
            raise ValueError(f"undeclared Phase 2A-S cell {key!r}")
        if str(item.get("architecture", "")) != ARCHITECTURE:
            raise ValueError("Phase 2A-S architecture drift")
        if str(item.get("panel_digest_sha256", "")) != PANEL_DIGEST_SHA256:
            raise ValueError("Phase 2A-S cell panel digest mismatch")
        ...
        if bool(item.get("neural_evolutionary_pressure", True)):
            raise ValueError("Phase 2A-S forbids neural evolutionary pressure")
        cells[key] = item

    if tuple(sorted(cells)) != tuple(sorted(expected_keys)):
        raise ValueError("Phase 2A-S cell set does not match the frozen design")

    for (depth, input_difference), item in cells.items():
        runs = item["runs"]
        if len(runs) != TRAININGS_PER_CELL:
            raise ValueError("every Phase 2A-S cell must contain exactly 48 trainings")
        ...
        for run in runs:
            ...
            if not 0 <= replicate < PAIRED_REPLICATES:
                raise ValueError("Phase 2A-S replicate index out of range")
            pair = (candidate_index, replicate)
            if pair in seen:
                raise ValueError("duplicate Phase 2A-S candidate/replicate training")
            seen.add(pair)
            if str(run["fingerprint"]) != fingerprints[candidate_index]:
                raise ValueError("Phase 2A-S training fingerprint mismatch")
            if int(run["source_seed"]) != source_seeds[candidate_index]:
                raise ValueError("Phase 2A-S source-seed provenance mismatch")
            if int(run["dataset_seed"]) != DATASET_SEEDS[replicate]:
                raise ValueError("Phase 2A-S dataset seed mismatch")
            if int(run["model_seed"]) != MODEL_SEEDS[replicate]:
                raise ValueError("Phase 2A-S model seed mismatch")
            ...
            if block[candidate_index] is not None:
                raise ValueError("duplicate Phase 2A-S candidate inside blocked cell")
        ...
        for difference in DIFFERENCES:
            for replicate in range(PAIRED_REPLICATES):
                ...
                if any(value is None for value in raw) or any(value is None for value in raw_null):
                    raise ValueError("Phase 2A-S incomplete blocked data")
    ...
    h4 = heterogeneity_by_depth[4]
    r4 = range_by_depth[4]
    ratios = {
        "H5_over_H4": (heterogeneity_by_depth[5] / h4) if h4 > 0 else None,
        "R5_over_R4": (range_by_depth[5] / r4) if r4 > 0 else None,
    }
    criteria = {
        "H3_gt_H4_gt_H5": heterogeneity_by_depth[3] > heterogeneity_by_depth[4] > heterogeneity_by_depth[5],
        "H5_over_H4_le_0_50": bool(ratios["H5_over_H4"] is not None and ratios["H5_over_H4"] <= 0.50),
        "R3_ge_R4_gt_R5": range_by_depth[3] >= range_by_depth[4] > range_by_depth[5],
        "R5_over_R4_le_0_75": bool(ratios["R5_over_R4"] is not None and ratios["R5_over_R4"] <= 0.75),
    }

    payload: dict[str, Any] = {
        "schema_version": 1,
        "experiment": "phase2as_depth_attenuation_aggregate",
        "panel_digest_sha256": PANEL_DIGEST_SHA256,
        "architecture": ARCHITECTURE,
        "depths": list(DEPTHS),
