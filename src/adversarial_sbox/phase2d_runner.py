"""Evolution runner for preregistered Phase 2D bounded persistence.

This module can access only the fresh fitness scorer. Held-out endpoint scoring is
intentionally separated from the evolutionary runner.
"""
...
from .phase2d_oracle import score_fitness_candidate
...
class Phase2DScoreLedger:
    ...
    def __init__(
        ...
        *,
        ...
        if int(budget) < 1:
            raise ValueError("Phase 2D score budget must be positive")
        self._scorer = scorer
        self._budget = int(budget)
        self._cache: dict[SBox, Phase2DScoreReceipt] = {}
    ...
    @property
    def receipts(self) -> tuple[Phase2DScoreReceipt, ...]:
    ...
    def score(self, sbox: Sequence[int], *, role: str = "selection") -> float:
        ...
        if self.score_count >= self._budget:
            raise RuntimeError("Phase 2D candidate-score budget exhausted")
        if role not in {"selection", "padding"}:
            raise ValueError(f"unsupported Phase 2D score receipt role {role!r}")
        payload = self._scorer(frozen)
        expected_fingerprint = fingerprint_sbox(frozen)
        if str(payload.get("purpose", "")) != "fitness":
            raise RuntimeError("Phase 2D fitness scorer purpose drift")
        if str(payload.get("fingerprint", "")) != expected_fingerprint:
            raise RuntimeError("Phase 2D fitness scorer fingerprint mismatch")
        if int(payload.get("training_count", -1)) != ORACLE_TRAININGS_PER_SCORE:
            raise RuntimeError("Phase 2D fitness training-count drift")
        score = float(payload.get("neural_advantage", float("nan")))
        if not math.isfinite(score):
            raise RuntimeError("Phase 2D fitness scorer returned non-finite score")
        payload_sha = str(payload.get("scientific_payload_sha256", ""))
        if not payload_sha:
            raise RuntimeError("Phase 2D fitness score receipt missing")
        frozen_payload = json.loads(json.dumps(payload, sort_keys=True))
        self._cache[frozen] = Phase2DScoreReceipt(
    ...
    def pad_to_exact_budget(self, candidates: Iterable[SBox], *, terminal: SBox) -> None:
        ...
        if self.score_count != self._budget:
            raise RuntimeError("insufficient audit-padding candidates for exact Phase 2D budget")
        if self.training_count != FITNESS_TRAININGS_PER_ARM_SEED:
            raise RuntimeError("Phase 2D exact fitness training budget drift")
...
def cutoff_order(
    ...
    *,
    ...
    cutoff: int,
    arm: str,
    oracle: Phase2DScoreLedger,
    shuffle_rng: random.Random | None,
    generation: int,
    stage: str,
    persistence: PersistenceLedger | None,
    audit_events: list[dict[str, Any]] | None = None,
    terminal_stage: bool = False,
) -> list[SBox]:
    """Apply one protected cutoff ordering and one-generation tag transition."""
    ...
    if stage not in {"shortlist", "survival"}:
        raise ValueError(f"unsupported Phase 2D stage {stage!r}")
    if not 1 <= int(cutoff) <= len(candidates):
        raise ValueError("Phase 2D cutoff outside candidate pool")

    base = _base_order(candidates, metrics=metrics, constraints=constraints)
    active_tags = (
        persistence.active(generation=int(generation), stage=stage)
        if persistence is not None and arm in {"OP1", "SP1"}
        else frozenset()
    )
    display_stage = "terminal_shortlist" if terminal_stage else stage
    ...
    def finish_tags() -> None:
        if persistence is not None and arm in {"OP1", "SP1"}:
            persistence.finish_stage(generation=int(generation), stage=stage)
    ...
    event: dict[str, Any] = {
        ...
        "group_end": int(end),
        "base_group": [_fp(candidate) for candidate in base[start:end]],
    }
        ...
        event.update(
            {
                ...
                "score_ordered_group": list(event["base_group"]),
                "final_group": list(event["base_group"]),
    ...
    if arm == "SP1":
        ...
        assigned = {
            ...
            for (candidate, _real_score), score in zip(scored, scores)
        }
    else:
        assigned = {candidate: float(score) for candidate, score in scored}

    if arm == "C":
        score_reordered = list(group)
    else:
        score_reordered = sorted(group, key=lambda candidate: assigned[candidate])

    if arm in {"OP1", "SP1"}:
        final_reordered = sorted(
            group,
            key=lambda candidate: (
                0 if _fp(candidate) in active_tags else 1,
                assigned[candidate],
            ),
        )
    else:
        final_reordered = list(score_reordered)

    score_global = [*base[:start], *score_reordered, *base[end:]]
    final_global = [*base[:start], *final_reordered, *base[end:]]
    score_selected = set(score_global[:cutoff])
    final_selected = set(final_global[:cutoff])
    score_entered = score_selected - before_selected
    actual_score_entered = score_entered & final_selected
    entered = final_selected - before_selected
    exited = before_selected - final_selected
    tag_created = sorted(_fp(candidate) for candidate in actual_score_entered)

    if (
        persistence is not None
        and arm in {"OP1", "SP1"}
        and not terminal_stage
    ):
        for candidate in actual_score_entered:
            persistence.create(
                fingerprint=_fp(candidate),
                generation=int(generation),
                stage=stage,
            )
    ...
    event.update(
        {
            ...
            "score_ordered_group": [_fp(candidate) for candidate in score_reordered],
            "final_group": [_fp(candidate) for candidate in final_reordered],
            ...
            "ordering_changed": bool(final_reordered != group),
            "score_caused_entered": tag_created,
            "entered": sorted(_fp(candidate) for candidate in entered),
            "exited": sorted(_fp(candidate) for candidate in exited),
            "tag_created": tag_created if arm in {"OP1", "SP1"} else [],
            "tag_used": bool(final_reordered != score_reordered),
            "selection_closed_after": bool(oracle.selection_closed),
        }
    )
    finish_tags()
    ...
    *,
    ...
    while len(batch) < PROPOSALS_PER_GENERATION:
        ...
        for record in generated:
            ...
            links.append(
                {
                    "proposal_fingerprint": _fp(record.sbox),
                    "parent_fingerprint": _fp(parent),
                }
            )
            added += 1
        ...
        if stalled > 512:
            raise RuntimeError("unable to generate globally unique Phase 2D proposals")
...
def _has_ancestor(candidate: str, ancestor: str, parent_map: dict[str, str]) -> bool:
    ...
    while current in parent_map:
        if current in seen:
            raise RuntimeError("cycle in Phase 2D realized parent map")
        seen.add(current)
...
def run_arm(
    *,
    ...
    """Run one C/O0/OP1/SP1 arm using fitness scores only."""

    if int(seed) not in EVOLUTION_SEEDS:
        raise ValueError(f"seed {seed} is not a frozen Phase 2D evolution seed")
    if not phase2d_evolution_seeds_are_fresh(EVOLUTION_SEEDS):
        raise RuntimeError("Phase 2D evolution seed provenance gate failed")
    if arm not in ARMS:
        raise ValueError(f"unsupported Phase 2D arm {arm!r}")
    ...
    oracle = Phase2DScoreLedger(scorer)
    persistence = PersistenceLedger()
    shuffle_rng = make_shuffled_control_rng(int(seed)) if arm == "SP1" else None
    ...
    for generation in range(EVOLUTION_GENERATIONS):
        ...
        for link in links:
            ...
            if child in parent_map:
                raise RuntimeError("duplicate Phase 2D proposal parent")
    ...
    if ledger.evaluations != CLASSICAL_BUDGET_PER_ARM_SEED:
        raise RuntimeError("Phase 2D classical budget drift")
    ...
    persistence.finish_stage(generation=EVOLUTION_GENERATIONS, stage="survival")
    ...
    payload: dict[str, Any] = {
        ...
        "persistence_state_at_end": persistence.snapshot(),
    }
